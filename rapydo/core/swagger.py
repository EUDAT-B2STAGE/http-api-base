...
from rapydo.core.confs import BACKEND_PACKAGE, CORE_DIR, USER_CUSTOM_DIR
...
class BeSwagger(object):
    ...
    def read_definitions(self, filename='swagger'):
        ...
        path = os.path.join(BACKEND_PACKAGE, CORE_DIR, 'models', file)
        data = load_yaml_file(path)

        # CUSTOM definitions
        file = '%s.%s' % (filename, YAML_EXT)
        path = os.path.join(BACKEND_PACKAGE, USER_CUSTOM_DIR, 'models', file)
